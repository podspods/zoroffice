...
export default function useRowActions(setOpenedModal: (openedModal: OpenedModal) => void) {
  return useMemo(() => [
    ...
    },
    {
      ...
      href: ([selectedRow]) => `translationMemory/${selectedRow.id}`
    },
    {
      ...RenameAction,
      ...
      disable(selectedRows) {
        ...
        return selectedRows.some((row) => {
          try {
            ...
            console.error('MergeAction.disable', error); // eslint-disable-line no-console
            return true;
          }
          return false;
        });
    ...
    {
      ...GroupPermissions,
      onClick: ([selectedRow]) => setOpenedModal({modalType: 'groupsPermissions', selectedRow: selectedRow})
    }
