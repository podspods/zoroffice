...
export async function commonFetch(api: string, options?: Options) {
  try {
    ...
    // Manage Errors
    const { message: cause } = contentType?.includes('application/json') ? await response.json() : {message: undefined };
    throw new Error(`${response.status} ${response.statusText}${cause ? ` - ${cause}` : ''}`, { cause });
