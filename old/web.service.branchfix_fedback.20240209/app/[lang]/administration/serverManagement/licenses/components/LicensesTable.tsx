...
function useLicenseModals({mutate}: {mutate: () => Promise<void>}) {
  ...
  const activateLicense = async (activationMethod: 'online' | 'offline' | 'phone', productKey: string, activationCode: string) => {
    try {
      ...
      console.error('activateLicense', error); // eslint-disable-line no-console
    }
    finally {
      await mutate();
    }
  ...
  const addProductKey = async (productKey: string) => {
    try {
      ...
      console.error('addProductKey', error); // eslint-disable-line no-console
    }
    finally {
      await mutate();
    }
  ...
  const deleteLicenses = async (licenses: License[]) => {
    try {
      ...
      console.error('deleteLicenses', error); // eslint-disable-line no-console
    }
    finally {
      await mutate();
    }
