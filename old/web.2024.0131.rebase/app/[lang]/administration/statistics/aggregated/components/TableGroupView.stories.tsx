...
function onChangeTypeStat(typeStat: TypeStat) {
  console.log('handleOnChangeTypeStat 8 ==>', typeStat);
}

function onChangePeriode(period: Period) {
  console.log('handleonChangePeriode 8 ==>', period);
}


const [currentTypeStat, setTypeStat] = useState<TypeStat>(statCategoryAggregated[0]);
const [period, setPeriod] = useState<Period>(getCurrentPeriod());


const meta: Meta<typeof TableGroupView> = {
  title: 'pages/Statistics/TableGroupView',
  component: TableGroupView,

  args: {
    onChangeTypeStat: onChangeTypeStat,
    typeStat: currentTypeStat,
  ...
  msw: {
    handlers: [
      ...
      rest.get(
        ...
        async (req, res, ctx) => {
          return res(ctx.delay(10), ctx.json(fakeDataGroupView.Administrator));
        }
      ),
      rest.get(
        Apis.statistics.byGroup('659d832714bc9e000da91579'),
        async (req, res, ctx) => {
...
export const Default: Story = {
  args: {
    onChangeTypeStat: handleOnChangeTypeStat,
    typeStat: { id: 1, label: 'Group', name: 'Aggregated by GROUPS' },
    onChangePeriode: handleOnChangePeriode,
    period: { monthNumber: '2023-1', monthString: 'January 2003' }
  }
};

/** ______________________________________________ msw ______________________________________________ */
