...
import TranslationResourceTable from './TranslationResourceTable';
  ...
  return (
    <>
      {lineList.map((oneLine, index) => {
        ...
        return <DisplayLine key={lineKey} {...oneLine} />;
      })}
    </>
  );
}

function expandText(service: Service): DisplayLineProps[] {
  if (!service?.name) return [];
  let itemList: DisplayLineProps[] = [];

  switch (service.name) {
    case ServiceName.MONGO_DB:
      itemList = expandMongoDb(service);
      break;
    case ServiceName.RABIT_MQ:
      itemList = expandRabbitMQ(service);
      break;
    case ServiceName.REDIS:
    case ServiceName.REDIS_NODE:
      itemList = expandRedis(service);
      break;
    case ServiceName.COMPUTING_NODE:
      itemList = expandComputingNode(service);
      break;
    ...
    default:
      itemList = expandDefault(service);
      break;
  }
  return itemList;
}
...
function expandComputingNode({ ...props }: Service): DisplayLineProps[] {
  return [
    ...
    { label: 'Translation Resources', value: '' },
    {
      label: '',
      innerHtml: props.translationResources && (
        <TranslationResourceTable data={props.translationResources} />
      ),
      value: '',
      leading: true
    }
...
function expandDefault({ ...props }: Service): DisplayLineProps[] {
  return [
    ...
    },
    {
      ...
      value: durationFromNow(props.lastTrsUpdate)
    },
    {
      label: 'Last Update',
      ...
      return [
        {
          ...
          leading: true,
          column: 1,
          status: oneNode.running ? Status.SUCCESS : Status.ERROR
        },
        {
          label: 'Running',
