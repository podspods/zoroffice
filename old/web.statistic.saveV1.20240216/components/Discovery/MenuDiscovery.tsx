...
import Toast from '@systran/react-components/lib/atoms/Toast';
...
const MenuDiscovery = ({children, ...props}: Props) => {
  ...
  const {toastMessage, updateToastMessage} = useContext(ToastMessageContext);
  ...
  return (
    <div className='content' style={{ display: 'flex', flex: '1', overflowX: 'hidden' }}>
      ...
      <Screen withoutMenu={isAnonymousUser}>
        <div style={{ display: 'flex', flex: '1', width: '100%' }}>
          ...
          <Toast severity={toastMessage?.status} displayCloseButton onClose={handleToastClose} open={!!toastMessage}>
            <>{i18n.t(toastMessage?.label || '')}</>
          </Toast>
          {children}
        </div>
      </Screen>
