...
export default function StatisticDetail() {
  ...
  const { result: rowList, totalChar, totalUser } = toDetailStatList(
    rawDetailStatList
  );
  ...
  return (
    <Box sx={{ width: '100%', margin: '2rem' }}>
      ...
      />
      <ModalStatSetting open={modalSettingVisible} onConfirm={() => null} onClose={() => setModalSettingVisible(false)} />
      {showDebug && (
        <ForDebug
          name={'for debug on '}
          dateRange={{
