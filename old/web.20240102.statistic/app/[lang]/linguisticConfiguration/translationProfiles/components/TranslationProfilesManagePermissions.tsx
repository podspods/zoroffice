      ...
      {translationResources.length > 0 && (
        <FormEntry label='Translation Resource'>
          <AppAutocomplete
            ...
            onChange={({value}: {value: string}) => props.onChange({translationResourceId: value})}
          />
        </FormEntry>
      )}
      {hasProfilePublic && (
        ...
        <>
          <FormEntry label='Users'>
            <AppAutocomplete
              ...
              value={usersSelected}
              onChange={(value) => props.onChange({users: value.map((user: UsersGroups) => user.value)})}
            />
          </FormEntry>
          <FormEntry label='Groups'>
            <AppAutocomplete
              ...
              onChange={(value) => props.onChange({groups: value.map((group: UsersGroups) => group.value)})}
            />
          </FormEntry>
        </>
      )}
