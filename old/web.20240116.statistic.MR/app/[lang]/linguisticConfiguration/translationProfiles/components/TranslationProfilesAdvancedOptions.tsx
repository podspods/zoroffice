...
type ResourcesOptionsType = {
  ...
  onChange: (value: {filterOptions: FilterOptionsPropType}) => void;
...
export type ResourcesOptionsRowsResults = {
  dictionaries?: ResourceOptionsMapField;
  normalizations?: ResourceOptionsMapField;
  translationMemories?: ResourceOptionsMapField;
  translationChoices?: ResourceOptionsMapField;
  sourceAnalysis?: ResourceOptionsMapField;
  countryLocalization?: ResourceOptionsMapField;
  rendering?: ResourceOptionsMapField;
  imperative?: ResourceOptionsMapField;
  pronouns?: ResourceOptionsMapField;
  styleParameters?: ResourceOptionsMapField;
};

const resourcesOptionsRows = (props: ResourcesOptionsType) => {
  const context = useContext(TranslationProfilesContext);
  const {source, target, lists, enabledNFA} = props;
...
export function ResourcesOptions(props: ResourcesOptionsType) {
  ...
  return <TranslationProfilesAdvancedOptions type='resourcesOptions' {...resourcesOptionsProps} />;
}

export function FilterOptions(props: ResourcesOptionsType) {
  const filterOptionsProps = {...props, rows: filterOptionsRows()};
  const onChange = (value: {filterOptions: FilterOptionsPropType}) => {
...
export default function TranslationProfilesAdvancedOptions({rows, disabled = false, source = '', ...props}: TranslationProfilesAdvancedOptionsType) {
  const context = useContext(TranslationProfilesContext);
  const onRowOptionChange = (value: any) => {
    props.onChange({[props.type]: value});
  };

  return (
    <Box id={`${props.type}_${props.id}`}>
      {Object.keys(rows).map((element, index) => {
        ...
        return (
          <TranslationProfilesRowOptions
            ...
            templateRows={fields}
            dataRows={props.dataRow?.[element as keyof ResourceOptions] || {}}
