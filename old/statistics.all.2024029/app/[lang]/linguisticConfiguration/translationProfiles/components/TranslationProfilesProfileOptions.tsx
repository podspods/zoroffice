    ...
    return (
      <Box mt={2}>
        {properties.map((element) => (
          <ObjectFieldTemplateStyled>{element.content}</ObjectFieldTemplateStyled>
        ))}
      </Box>
    );
  };
    ...
    <Box>
      {Boolean(profileId) && ( // if there is no id yet, it is a creation form and there is no id to show
        <Grid container spacing={2} className='form-group' alignItems='center' mb='15px'>
          <Grid item md={2.1} display='flex' justifyContent='flex-end'>
      ...
      <Box style={{marginBottom: '15px'}}>
        ...
        {!profileData.flowProfile && (
          <Grid container spacing={2} alignItems='center' style={{wordBreak: 'break-all'}}>
            <Grid item md={2.1} display='flex' justifyContent='flex-end'>
