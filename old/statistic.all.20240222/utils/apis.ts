    ...
    if (filterValue !== undefined) {
      if (Array.isArray(filterValue)) {
        filterValue.forEach(
          (value?: string) =>
            value !== undefined &&
            queryParams.append(`eleFilters[${filterName}][]`, value)
        );
      } else {
...
const Apis = {
  ...
  fileTranslation: {
    ...
    downloadFiles: (ids: string[]) =>
      `/node/fileTranslation/download?${new URLSearchParams(
        ids.map((id) => ['ids', id])
      ).toString()}`,
    ...
    cache: (type?: string) =>
      `/node/session/translate/languages${
        type ? '?' + new URLSearchParams({ type: 'file' }) : ''
      }`
  },
  filePostEditor: {
  ...
  speechTranslation: {
    ...
    download: ({
      ...
      `/node/speechTranslation/download/${mode}${
        ids.length === 1
          ? `/${ids[0]}`
  ...
  corpus: {
    list: ({ directory }: { directory: string }) =>
      `/node/corpusManager/list?${new URLSearchParams({
        directory
      }).toString()}`,
    details: ({ id }: { id: string }) => `/node/corpusManager/details/${id}`,
    segment: {
      update: ({ id }: { id: string }) =>
        `/node/corpusManager/segment/update/${id}`,
      add: ({ id }: { id: string }) => `/node/corpusManager/segment/add/${id}`,
      delete: ({ id }: { id: string }) =>
        `/node/corpusManager/segment/delete/${id}`,
      list: ({ id, target }: { id: string; target?: string }) =>
        `/node/corpusManager/segment/list/${id}${
          target ? '?tgtLanguage=' + target : ''
        }`
    },
    ...
    download: ({
      ...
      corpusId: string;
      ownerId?: string;
      format: 'text/bitext' | 'application/x-tmx+xml';
    }) => {
  ...
  resources: {
    tm: (type: 'users' | 'groups', resourceId: string) =>
      `/node/resources/tm/${type}/${resourceId}`,
    permissions: (type: 'users' | 'groups') =>
      `/node/resources/tm/${type}/permission`
  },
  feedback: {
  ...
  dictionary: {
    list: ({ type }: { type: 'UD' | 'NORM' }) =>
      `/node/gdict/dictionary/list?dictType=${type}`,
    ...
    upload: ({ id, accountId }: { id: string; accountId: string }) =>
      `/node/gdict/dictionary/import?dictId=${id}&accountId=${accountId}`,
    users: (resourceId: string) =>
      `/node/resources/dictionary/users/${resourceId}`,
    groups: (resourceId: string) =>
      `/node/resources/dictionary/groups/${resourceId}`,
    permissions: (type: 'users' | 'groups') =>
      `/node/resources/dictionary/${type}/permission`,
  ...
  profiles: {
    ...
    getListActivedProfile: (queryParams: URLSearchParams) =>
      `/node/profiles?${queryParams}`,
    getLanguagePair: ({ onlyRunning }: { onlyRunning: boolean }) =>
      `/node/lps?onlyRunning=${onlyRunning}`,
    getDictionaries: ({ source, target }: { source: string; target: string }) =>
      `/node/resources/dictionary/list?source=${source}&target=${target}`,
    getTranslationMemories: ({
      ...
      source: string;
      target: string;
    }) => `/node/corpusManager/names?source=${source}&target=${target}`,
    getEngines: ({
      source,
      target,
      onlyLastVersion
      ...
      `/node/resources/engine/list?source=${source}&target=${target}&onlyLastVersion=${onlyLastVersion ??
        true}`,
    getGroupListLight: (withRoles: boolean) =>
      `/node/admin/groups/light?withRoles=${withRoles}`,
    getAccountListLight: (queryParams: URLSearchParams) =>
      `/node/admin/users/light?${queryParams}`,
  ...
  group: {
    ...
    getRoleListLight: ({ limit }: { limit: number }) =>
      `/node/admin/roles/light?limit=${limit}`,
    getUserListLight: ({ limit }: { limit: number }) =>
      `/node/admin/users/light?${limit}`,
    pushUserToGroup: (groupId: string, userId: string) =>
      `/node/admin/group/${groupId}/user/push/${userId}`,
    removeUserFromGroup: (groupId: string, userId: string) =>
      `/node/admin/group/${groupId}/user/remove/${userId}`,
    pushRoleToGroup: (groupId: string, roleId: string) =>
      `/node/admin/group/${groupId}/role/push/${roleId}`,
    removeRoleFromGroup: (groupId: string, roleId: string) =>
      `/node/admin/group/${groupId}/role/remove/${roleId}`,
    setMaximumCharactersConsumption: (groupId: string) =>
      `/node/admin/group/${groupId}/limits`
  },
  user: {
    ...
    getGroupListLight: ({
      ...
      limit: number;
      withRoles: boolean;
    }) => `/node/admin/groups/light?limit=${limit}?withRoles=${withRoles}`,
  ...
  translationResources: {
    ...
    activeResources: (id: string) =>
      `/node/translationResource/${id}/routes/activate/all`,
    deactivateResources: (id: string) =>
      `/node/translationResource/${id}/routes/deactivate/all`,
    deleteResources: (id: string) => `/node/translationResource/delete/${id}`,
    getDowngradeList: (id: string) =>
      `/node/translationResource/downgrade/list/${id}`,
    getUpgradeList: (id: string) =>
      `/node/translationResource/upgrade/list/${id}`,
    upgradeResource: (id: string) => `/node/translationResource/upgrade/${id}`,
    downgradeResource: (id: string) =>
      `/node/translationResource/downgrade/${id}`,
    addRoute: '/node/route/add',
    getTranslationResourceDetail: (id: string, mode = 'ses') =>
      `/node/translationResource/${id}?mode=${mode}`,
    getAuthorizeConfigTrOption: '/node/translationResources/options',
    installFollowingNode: (id: string) =>
      `/node/translationResource/install/${id}`,
    uninstallFollowingNode: (id: string) =>
      `/node/translationResource/uninstall/${id}`,
    updateInstance: (id: string) =>
      `/node/translationResource/instances/update/${id}`
  },
  statistics: {
    fullView: '/node/activity/details',
    bySession: '/node/activity',
    ...
    personal: '/node/activity/user/personal/details'
  },
  information: {
