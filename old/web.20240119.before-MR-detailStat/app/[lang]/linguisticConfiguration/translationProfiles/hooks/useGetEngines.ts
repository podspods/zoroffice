...
export default function useGetEngines({source, target, onlyLastVersion}: Props) {
  ...
  const {data: enginesData = {}, ...result} = useSWR(Apis.profiles.getEngines({source, target, onlyLastVersion}), null, {refreshInterval: refreshInterval === 'Never' ? 0 : refreshInterval * 1000});
