...
function useLicenseModals({mutate}: {mutate: () => Promise<void>}) {
  const { globalSettings } = useContext(MyContext);
    ...
    case 'activate':
      modal = (
        <ActivateLicenseForm
          ...
          installationId={globalSettings?.installationId}
...
function renderStatusBadge(activated: boolean, row: License) {
  ...
  let type: LicenseStatus = 'success';
  if (!row.valid) {
    type = 'error';
  }
  else if (!activated) {
  ...
  return (
    <StatusBadge title={t(text)} type={type} text={t(text)} />
  );
}
    ...
    <>
      ...
      <Box style={{width: '100%'}}>
        ...
        <SimpleTable
          ...
          pagination
          getDetailPanelContent={LicenseBox}
        />
      </Box>
    </>
  );
