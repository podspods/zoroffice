...
import { convertService, doSearch } from './services';
...
export const searchChange = action(
  ...
  (store, event) => {
    ...
    if (!isBusy) {
      ...
      setMessage('searchChange: ' + event.target.value);
  ...
  (store, rawData) => {
    ...
    if (!isBusy && isDataReady) {
      ...
      if (rawData && rawData.services) {
        ...
        if (searchText) {
          console.log('newFilteredList 275==>');
          newFilteredList = doSearch(searchText, newServiceList);
        } else {
          console.log('newFilteredList 278 ==>');
