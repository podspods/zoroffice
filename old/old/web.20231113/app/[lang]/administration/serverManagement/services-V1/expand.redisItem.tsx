...
import { displayByte } from './services';

export function expandRedis({ ...props }: Service): DisplayLineProps[] {
  if (!props.data)
    return [
      {
  ...
  return [
    ...
    { label: BITS, value: redisData.Server.arch_bits + ' remove ??'},
    {
      label: UPTIME,
    ...
    },
    {
      ...
      value: redisData.CPU.used_cpu_user
        ? redisData.CPU.used_cpu_user.toString() + ' % ' // arrondir Zéro décimal
        : undefined,
      column: 1
    },
    {
      ...
      value: redisData.CPU.used_cpu_sys_children
        ? redisData.CPU.used_cpu_sys_children.toString() + ' % ' // arrondir Zéro décimal
        : undefined,
      column: 1
    },
    {
      ...
      value: redisData.CPU.used_cpu_user_children
        ? redisData.CPU.used_cpu_user_children.toString() + ' % ' // arrondir Zéro décimal
        : undefined,
      column: 1
    },
