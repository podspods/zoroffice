...
function getRequestBody(profileData: ProfileAddType) {
  ...
  requestBody.public = profileData.profileOptions.public ?? false;
...
export default function TranslationProfilesAdd({openedModal, onClose}: Props) {
  ...
  const onCreateProfile = async () => {
    const requestBody = getRequestBody(profileData);
  ...
  if (profileData.step === 'PROFILE_OPTIONS') {
    modal = <ProfileOptionsModal onChange={onChange} onCreateProfile={onCreateProfile} onClose={onClose} />;
  }
...
type TProfileOptionsModal = {
  onChange: (e: ProfileOnChangeOptionsType) => void;
  onCreateProfile: () => Promise<void>;
  onClose: () => void;
};

function ProfileOptionsModal(props: TProfileOptionsModal) {
  const {t} = useTranslation();
  const {data: {connectedUserId}} = useContext(TranslationProfilesContext);
  const {data: profileData, setData: setProfileData, clearData: clearProfileData} = useContext(TranslationProfilesAddContext);
  const {hasProfileSharedPermission, hasProfilePublicPermission} = getProfilesPermission();
  ...
  const onConfirm = async () => {
    ...
    if (!hasProfileSharedPermission && !hasProfilePublicPermission) { // if user don't have share or public permission, we don't need to display the permission modal
      const requestBody = getRequestBody(profileData);
      requestBody.users = [connectedUserId];
      await PostApi.createProfile(requestBody);
      props.onClose();
      return;
    }
  ...
  return (
    <ConfirmModal width='extraLarge' title={t('Create a profile')} open onClose={onClose} onConfirm={onConfirm}>
...
function ProfileOptionsManagePermissionsModal(props: {
  ...
  return (
    <ConfirmModal width='extraLarge' title={t('Manage Permissions')} open onClose={onClose} onConfirm={props.onConfirm}>
