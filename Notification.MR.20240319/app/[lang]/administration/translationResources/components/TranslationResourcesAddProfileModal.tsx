...
import {Engine, ModelOptions, OptionsType, ProfileOnChangeOptionsType, ResourceOptions, Selectors} from 'app/[lang]/linguisticConfiguration/translationProfiles/context/TranslationProfilesAddContext';
...
export default function TranslationResourcesAddProfileModal({onClose, resourceData}: Props) {
  ...
  const [requestData, setRequestData] = useState<RequestDataType>({
    ...
    selectors: {...resourceData.selectors},
    engineSelected: {
      id: resourceData.id,
...
const Permissions = ({requestData, onClose, onChangeProfileOptions}: TProfileOptionsProp) => {
  ...
  const onCreateProfile = async () => {
    const {dictionaries, normalizations, translationMemories} = requestData.resourcesOptions;
    const requestBody = {
      ...requestData,
      resources: {
        engine: requestData.engineSelected,
        userDict: dictionaries?.dictionaries.map((dictionary) => {
          return {
            id: dictionary.id,
        ...
        normalization: normalizations?.normalizations.map((normalization) => {
          return {
            id: normalization.id,
        ...
        translationMemory: {
          corpusIds: translationMemories?.translationMemories.map((translationMemory) => {
