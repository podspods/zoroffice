...
const Apis = {
  ...
  fileTranslation: {
    ...
    profiles: (params?: {source: string, target: string}) => `/node/translate/supportedProfiles${params ? '?' + new URLSearchParams(params) : ''}`,
    ...
    features: (params?: {source?: string, target?: string, profileId?: string}) => `/node/translate/supportedFeatures${params ? '?' + new URLSearchParams(omitBy(params, isUndefined)) : ''}`,
    selectors: '/node/translate/supportedSelectors',
    cache: (type?: string) => `/node/session/translate/languages${type ? '?' + new URLSearchParams({type}) : ''}`
  },
  filePostEditor: {
    ...
    saveSentence: '/node/postEditor/saveSentence',
  ...
  speechPostEditor: {
    stream: (id: string) => `/node/speechTranslation/postEditor/stream/${id}`,
    updateTranslation: '/node/speechTranslation/postEditor/updateTranslation'
  },
  speechTranslation: {
  ...
  feedback: {
    ...
    listTM: (params?: {source: string, target: string}) => `/node/corpusManager/list?sourceLanguageCode=${params?.source || ''}&targetLanguageCode=${params?.target || ''}`,
  ...
  dictionary: {
    ...
    entry: {
      list: '/node/gdict/entry/list',
      update: '/node/gdict/entry/update',
      delete: '/node/gdict/entry/delete',
      add: '/node/gdict/entry/add'
    }
  },
  password: {
  ...
  user: {
    ...
    delete: ({id}: {id: string}) => `/node/admin/user/delete/${id}`,
    add: '/node/signup',
    activateUser: ({id}: {id: string}) => `/node/admin/user/enable/${id}`,
    deactivateUser: ({id}: {id: string}) => `/node/admin/user/disable/${id}`,

    getUser: (id: string) => `/node/admin/user/${id}`,
    getGroupListLight: ({limit, withRoles}: {limit: number, withRoles: boolean}) => `/node/admin/groups/light?limit=${limit}?withRoles=${withRoles}`,
    getRoleListLight: ({limit}: {limit: number}) => `/node/admin/roles/light?limit=${limit}`,
    pushGroupToUser: (userId: string, groupId: string) => `/node/admin/group/${groupId}/user/push/${userId}`,
    removeGroupFromUser: (userId: string, groupId: string) => `/node/admin/group/${groupId}/user/remove/${userId}`,
    pushRoleToUser: (userId: string, roleId: string) => `/node/admin/user/${userId}/role/push/${roleId}`,
    removeRoleFromUser: (userId: string, roleId: string) => `/node/admin/user/${userId}/role/remove/${roleId}`,
    updateUserData: ({userId}: {userId: string}) => `/node/admin/user/${userId}`,

    listApiKeys: ({userId}: {userId: string}) => `/node/user/${userId}/apiKey/list`,
    generateApiKey: ({userId}: {userId: string}) => `/node/user/${userId}/apiKey/generate`,
    editApiKey: ({userId, apiKey}: {userId: string, apiKey: string}) => `/node/user/${userId}/apiKey/edit/${apiKey}`,
    disableApiKey: ({userId, apiKey}: {userId: string, apiKey: string}) => `/node/user/${userId}/apiKey/disable/${apiKey}`,
    personalApiKeys: '/node/user/apiKey/list',

    getClientCredentials: ({userId}: {userId: string}) => `/node/user/${userId}/clientCredentials`,
    deleteClientCredential: ({clientId, rat}: {clientId: string, rat: string}) => `/node/user/oidc/reg/${clientId}?access_token=${rat}`,
    createClientCredentials: '/node/user/oidc/reg',
    personalClientCredentials: '/node/user/clientCredentials',

    preferences: '/node/user/preferences',
    setPreferences: '/node/user/preferences',
    updatePreferences: '/node/user/preferences',

    getUserActiveTokens: ({userId}: {userId: string}) => `/node/user/${userId}/activeTokens`,
    personalActiveTokens: '/node/user/activeTokens',
    revokeTokens: '/node/user/oidc/token/revocation',

    getPersonalUser: '/node/user/personal',
    updatePersonalData: '/node/user/personalData',

    changePassword: '/node/user/password/change',
    resetPassword: (id: string) => `/node/resetPassword/${id}`
  },
