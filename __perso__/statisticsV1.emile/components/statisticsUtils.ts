...
export function toDetailStatList(
  ...
  const result: DetailStatistic[] = rawDetailStatList
    ...
    .map((oneRow) => toDetailStat(oneRow));
  return {
    result: result,
    ...
    totalUser: countDistinctUser(result)
  };
}
...
// ????????????????????????? tout refaire avec des colonnes séparé et nom de colonne == sort et filter ????????????
...
export function userViewFilter(field: string): string {
  switch (field) {
    case 'filter':
      return field;
    ...
    case 'langPair':
    case 'source':
      return 'eleFilters[source]';

    case 'target':
      return 'eleFilters[target]';

    case 'requestSuccess':
    case 'requestSuccessPercent':
      return 'eleFilters[success]';
    case 'segment':
      return 'eleFilters[nbSegments]';
    case 'segmentInCache':
    case 'segmentInCacheHit':
      return 'eleFilters[nbCacheHits]';
    case 'tus':
      return 'eleFilters[nbTus]';
    case 'tusFail':
      return 'eleFilters[nbTusFailed]';
    case 'elapsedTime':
      return 'eleFilters[elapsedTime]';
    case 'character':
      return 'eleFilters[nbCharacters]';
    case 'characterInCache':
      return 'eleFilters[nbCharactersCacheHits]';
    default:
      return `eleFilters[${field}]`;
  }
}

export function groupViewFilter(field: string): string {
  return userViewFilter(field);
}

export function profileViewFilter(field: string): string {
  switch (field) {
    case 'filter':
      return field;

    case 'profileNameAgg':
      return 'eleFilters[profileName]';
    case 'langPair':
    case 'source':
      return 'eleFilters[sourceLanguage]';

    case 'target':
      return 'eleFilters[targetLanguage]';

    case 'userCount':
      return 'eleFilters[totalAccounts]';
    case 'request':
      return 'eleFilters[total]';
    case 'requestSuccess':
    case 'requestSuccessPercent':
      return 'eleFilters[totalSuccess]';
    ...
    case 'segmentInCacheHit':
      return 'eleFilters[nbCacheHits]';
    case 'tus':
      return 'eleFilters[totalNbTus]';
    case 'tusFail':
      return 'eleFilters[totalNbTusFailed]';
    case 'elapsedTime':
      return 'eleFilters[totalElapsedTime]';
    case 'character':
      return 'eleFilters[totalNbCharacters]';
...
export function detailViewFilter(field: string): string {
  switch (field) {
    case 'filter':
      return field;
    ...
    case 'date':
    case 'moment':
      return 'eleFilters[date]';
    case 'userName':
      return 'eleFilters[accountName]';
...
export function personalViewFilter(field: string): string {
  return userViewFilter(field);
}
